...
export async function decryptSources_v1(epID, id, name, type, fallback) {
  try {
    ...
    if (fallback) {
      ...
      // Use Regex instead of Cheerio for ~10x faster performance on Cloudflare Workers (Fixes 1102 Error)
      const dataIdMatch = data.match(/id="megaplay-player"[^>]+data-(?:id|ep-id)="([^"]+)"/);
      const dataId = dataIdMatch ? dataIdMatch[1] : null;

      if (!dataId) {
        console.warn(`[Decryptor] data-id not found for epID: ${epID}`);
        throw new Error("Source ID not found in Megaplay response");
      }

      const { data: decryptedData } = await http.get(
      ...
      console.log("[Decryptor] decryptedData:", JSON.stringify(decryptedData, null, 2));
      decryptedSources = decryptedData;
    } else {
      const { data: sourcesData } = await http.get(
      ...
      const { data: rawSourceData } = await http.get(
